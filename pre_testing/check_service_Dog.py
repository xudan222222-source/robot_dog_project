...
class DogServiceChecker:
    ...
    def check_services(self) -> bool:
        ...
        if all_pass:
            print(
                f"[PASS] 共检测 {enabled_count} 个 enabled 服务，"
                f"全部 active"
            )
        else:
            print(
                f"[FAIL] 共检测 {enabled_count} 个 enabled 服务，"
                f"存在异常服务"
            )
...
if __name__ == "__main__":
    ...
    DogService_checker = DogServiceChecker(
    ...
    try:
        if DogService_checker.connect_ssh():
            DogService_checker.check_services()

    finally:
        DogService_checker.close_ssh()